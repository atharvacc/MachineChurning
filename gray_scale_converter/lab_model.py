...
from lab_gray_converter.utils import list_files1, load_from_dir,imread_grayscale
import os
...
class lab_gray_scale_model:
    ...
    def preprocess_data(self, directory, extension):
        _, data_gray = load_from_dir(directory, extension)
    ...
    def train(self):
        ...
        self.model.save(model_save_dir + "./model/lab_model/{}.hd5".format(datetime.now().strftime('%Y-%m-%d %H:%M')))

    def predict(self):
        if not os.path.exists(self.test_save_dir):
            os.makedirs(self.test_save_dir)
        for img_name in os.listdir(self.test_dir):
            img = (1.0/255) * imread(self.test_dir + "/" + img_name)
            img = skimage.color.rgb2lab(img)
            img = img[:,:,0]
            pred = self.model.predict(img.reshape(1,512,512,1)).reshape(512,512,2)
            pred = (pred * 256)- 128
            pred = skimage.color.lab2rgb( np.concatenate((img,pred), axis =2 ))
            imsave("{}/pred_{}".format(self.test_save_dir, img_name), pred.astype(np.uint8))
        return  
