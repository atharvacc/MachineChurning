...
def main():
    ...
    if (parser.parse_known_args()[0].model_name == "base_gray" or parser.parse_known_args()[0].model_name == "lab_gray"): # If using the base_gray scale or lab model 
        ...
        if (parser.parse_known_args()[0].train_or_test == "train"):
            ...
            --loadSize 512 --fineSize 512 --display_winsize 512 --name test_cyclegan --model cycle_gan --netG resnet_fpn" .format(train_dir = parser.parse_args().train_dir),
            lr = parser.parse_args().lr, batch_size = parser.parse_args().batch_size, model_save_dir = parser.parse_args().model_save_dir, epochs = parser.parse_args().epochs,
